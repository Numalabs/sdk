...
const PLATFORM_URL = process.env.Numalabs_API_URL || 'https://api.Numalabs.ai'
const RUNTIME_URL = process.env.Numalabs_RUNTIME_URL || 'https://agents.Numalabs.ai'
...
export interface AgentOptions {
  ...
  /**
   * The Numalabs API key for authentication.
   * Can also be provided via Numalabs_API_KEY environment variable.
   */
  apiKey?: string
...
export class Agent {
  ...
  /**
   * The Numalabs API key used for authentication.
   * Can be provided in options or via Numalabs_API_KEY environment variable.
   * @private
   */
  private apiKey: string

  /**
   * Axios instance for making requests to the Numalabs API.
  ...
  /**
   * Axios instance for making requests to the Numalabs Runtime API.
   * Pre-configured with base URL and authentication headers.
   * @protected
   */
   ...
   * @throws {Error} If Numalabs API key is not provided in options or environment
   */
  constructor(private options: AgentOptions) {
    ...
    this.apiKey = this.options.apiKey || process.env.Numalabs_API_KEY || ''

    if (!this.apiKey) {
      throw new Error(
        'Numalabs API key is required. Please provide it in options or set Numalabs_API_KEY environment variable.'
      )
    }
  ...
  /**
   * Calls an integration endpoint through the Numalabs platform.
   * This method allows agents to interact with external services and APIs that are integrated with Numalabs.
   *
